...
func prepareFinalInputVariableValue(addr addrs.AbsInputVariableInstance, raw *InputValue, cfg *configs.Variable) (cty.Value, tfdiags.Diagnostics) {
	...
	if cfg.Ephemeral {
		// An ephemeral input variable always has an ephemeral value,
		// inside the module, even if the value assigned to it from outside
		// is not. This is a useful simplification so that module authors can
		// be explicit about what guarantees they are intending to make
		// (regardless of current implementation details). Marking an
		// output value as not ephemeral when it was before is always a
